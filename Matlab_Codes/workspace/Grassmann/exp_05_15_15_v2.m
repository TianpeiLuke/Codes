...
param.alphaT = 1e-2; %0.8*pi*beta*5e-4; %3e-1; %1; %1e-3;%5e-4\
param.alpharr = 1/4;
param.dostop = 0;
...
%  title(sprintf('Reconstruction error for learned subspace'))
 ...
 figure(7)
 history_opt = history_out{minidx};
 X = Xarray{minidx};
 Rs = X'*Y;
 [UR,SR, VR] = svd(Rs);
 Xtst = X*UR;
 ...
 W = Xtst'*Y;
 Latent = Y*W';
 plot(Latent(:,1), Latent(:,2),'ob','MarkerSize',10)
 xlabel('1st direction','Fontsize',18);
 ylabel('2nd direction','Fontsize',18);
