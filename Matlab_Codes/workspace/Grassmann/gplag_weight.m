function [X, Gamma, history, epsilon] = gplag_weight(X0, G0, Y, Ky, param, option, sel)
...
%% Initialization
%g0const = 1e-2;
gamma0 = diag(G0); %g0const*ones(k,1);
gamma = gamma0;
Gamma = diag(gamma);
%lambda = 1e-3;
%param.lambda = lambda;
...
[UR,SR, VR] = svd(R);
Xtst = X*UR;
epsilon = k-norm(Xtst'*Us(:,1+dt:k+dt),'fro'); %measure the distance btw subspaces
history.epsilon(1) = epsilon;
[~,Sdiff,~] = svd(Xtst'*Us(:,1+dt:k+dt));
history.UUSigma(:,1) = diag(Sdiff); 
end
objval = neglogP(Ky, X, Gamma, param, option); 
...
%    
   ...
   [UR,SR, VR] = svd(R);
   Xtst = X*UR;
   epsilon = k-norm(Xtst'*Us(:,1+dt:k+dt),'fro');
   [~,Sdiff,~] = svd(Xtst'*Us(:,1+dt:k+dt));
