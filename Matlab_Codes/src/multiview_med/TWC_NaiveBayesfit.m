function obj =  TWC_NaiveBayesfit(obj, training,group,  varargin)
...
if isscalar(obj.Dist)
    switch obj.Dist{:}
        case 'mn'
            obj =  mnTWCfit(obj,training, gindex);
        case 'normal'
...
function obj =  mnTWCfit(obj,training, gidx)
d = size(training,2);
for k = obj.NonEmptyClasses
    groupI = (gidx == k); % Complement features
    if sum(groupI) == 0
        error(message('stats:NaiveBayes:fit:NoDataInEachClass'));
    end
    
    pw = sum(training(groupI,:),1);
    pw = (pw+obj.alpha)/(sum(pw)+sum(obj.alpha)); % normalization
    %Add one displacement for  each feature to avoid zero probability
