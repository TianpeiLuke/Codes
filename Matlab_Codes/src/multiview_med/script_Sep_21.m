...
nV = 2;
 ...
 BayesError = zeros(1,TotalSet);
 iset = 1;
 %%
for iset = 1:TotalSet
 
rho_Bayes = 1/8*(mu_22(iset,:)- mu_21(iset,:))*((0.5*Sigma22 + 0.5*Sigma21)\(mu_22(iset,:)- mu_21(iset,:))')...
    +0.5*log(det((0.5*Sigma22 + 0.5*Sigma21)))-1/4*log(det(Sigma22))-1/4*log(det(Sigma21));
...
%     
BayesError(iset) = 0.5*(0.5*(1-sqrt(1-exp(-2*rho_Bayes)))) + 0.5*(0.5*exp(-rho_Bayes));
...
%% parameter setting
    Traindata.X_U{1} = X_U(:,:,1) ;
    Traindata.X_U{2} = X_U(:,:,2) ;
    Traindata.y_U = y_U ;
    Traindata.X_L{1} = X_L(:,:,1) ;
    Traindata.X_L{2} = X_L(:,:,2) ;
    Traindata.y_L = y_L ;
    Traindata.nU = nU;
    Traindata.nL = nL;
    Traindata.nV = nV;
    ...
    Testdata.X_Tst{1} = X_tst(:,:,1);
    Testdata.X_Tst{2} = X_tst(:,:,2);
    Testdata.y_Tst = y_tst;
    Testdata.nTst  = nTst;
    Testdata.d = [2,2];

    param.regParam = 1;
    param.kernelMethod = [{'rbf'}, {'rbf'}];  
    param.kernelParm   = [sqrt(1/(2*0.5^2)), sqrt(1/(2*0.5^2))];
    param.maxIterOut = 20;
    param.threOut = 1e-3;
    param.maxIterMAP = 50;
    param.threMAP = 1e-4;
    param.sigmaPri = 1.5;
    param.mode = 1;
    param.Distribution = [{'normal'}, {'normal'}];
...
figure(2)
color = [{'-r'},{'-b'},{'-g'},{'-m'},{'-.b'},{'-.g'}];
legendstr = [{sprintf('MV-MED Bayes Error = %.1f',BayesError(TotalSet)*100)}];
...
legendstr = [legendstr, {sprintf('Co-training Bayes Error = %.1f',BayesError(TotalSet)*100) }];
...
legendstr = [legendstr, {sprintf('MV-MED Bayes Error = %.1f %%',BayesError(iset)*100 )}];
errorbar(r_a*100, mean(accuracy_ct_array(:,:,iset),2), std(accuracy_ct_array(:,:,iset),[],2),color{iset},'Linewidth',1.5);
legendstr = [legendstr, {sprintf('Co-training Bayes Error = %.1f %%',BayesError(iset)*100)}];
end
hold off;
xlabel 'unlabel ratio (%)'
ylabel 'Accuracy(%)'
...
figure(1)
color = [{'-r'},{'-b'},{'-g'},{'-m'},{'-.b'},{'-.g'}];
legendstr = [{sprintf('MV-MED unlabeled rate = %.1f',r_a(1)*100)}];
Temp_Acc_mv = squeeze(accuracy_mv_array(1,:,:));
Temp_Acc_ct = squeeze(accuracy_ct_array(1,:,:));
errorbar(BayesError*100, mean(Temp_Acc_mv,1), std(Temp_Acc_mv,[],1),color{1},'Linewidth',2);
hold on;
errorbar(BayesError*100, mean(Temp_Acc_ct,1), std(Temp_Acc_ct,[],1),color{2},'Linewidth',2);
legendstr = [legendstr, {sprintf('Co-training unlabeled rate = %.1f',r_a(1)*100) }];

for ir=[3, 5] %TotalSet-1:-2:1
Temp_Acc_mv = squeeze(accuracy_mv_array(ir,:,:));
Temp_Acc_ct = squeeze(accuracy_ct_array(ir,:,:));

errorbar(BayesError*100, mean(Temp_Acc_mv,1), std(Temp_Acc_mv,[],1),color{ir},'Linewidth',1.5);
legendstr = [legendstr, {sprintf('MV-MED unlabeled rate = %.1f %%',r_a(ir)*100 )}];
errorbar(BayesError*100, mean(Temp_Acc_ct,1), std(Temp_Acc_ct,[],1),color{ir+1},'Linewidth',1.5);
legendstr = [legendstr, {sprintf('Co-training unlabeled rate = %.1f %%',r_a(ir)*100)}];
end
hold off;
xlabel 'Bayes Error (%)'
ylabel 'Accuracy(%)'
legend(legendstr)
axis([0,max(BayesError*100)+5,0,100])
grid on
