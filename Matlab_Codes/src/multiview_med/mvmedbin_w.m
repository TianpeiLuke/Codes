function [accuracy, errorlist, dev_tst, prob_tst, dev_trn, prob_trn, wjoint...
...
%   dev_tst:  nTst x nV decision value for test samples
%
%  prob_tst:  nTst x 1, consensus probability on test samples
% 
%
...
for i=1:nV
  ...
  w_map0(:,i) = (model{i}.sv_coef' * full(model{i}.SVs))';
end
...
while( outflag && iout< maxIter)
  ...
  display(sprintf('-- inner loop: t=%d',t));
   ...
   % update f_map1, f_map2
    ...
    for i=1:nV
      ...
      ncount = 0;
      while(~idump)
       fmap_temp(:,i) = (1-sigma2*mu(i))*fmap(:,i)+ ...
          mu(i)*sigma2*K_u{i}*((pinv(M(:,:,i)) + K_u{i})\fmap(:,i))...
          + mu(i)*sigma2^2*K_u{i}*(q-p(:,i)) ...
         - mu(i)*sigma2^2*K_u{i}*((pinv(M(:,:,i)) + K_u{i})\K_u{i}*(q-p(:,i)) );
     
       v_temp(:,i) = 1/2*1./(1+cosh(fmap_temp(:,i)));    
       M_temp(:,:,i) = diag(v(:,i));
       p_temp(:,i) =  sigmoid(fmap_temp(:,i));  
       
       ratio = norm(fmap_temp(:,i) - sigma2*K_u{i}*(q-p_temp(:,i)))/norm(fmap(:,i) - sigma2*K_u{i}*(q-p(:,i)));
       
       if 1- ratio > epsilon_t
         idump = 1;  
       end
       ncount = ncount+1;
      ...
      % ---- update ---------------------
      fmap(:,i) = (1-sigma2*mu(i))*fmap(:,i)+ ...
          mu(i)*sigma2*K_u{i}*((pinv(M(:,:,i)) + K_u{i})\fmap(:,i))...
          + mu(i)*sigma2^2*K_u{i}*(q-p(:,i)) ...
          - mu(i)*sigma2^2*K_u{i}*((pinv(M(:,:,i)) + K_u{i})\K_u{i}*(q-p(:,i)) );
      
      fpred(:,i) = (1-sigma2*mu(i))*fpred(:,i)+ ...
          mu(i)*sigma2*K_ul{i}'*((pinv(M(:,:,i)) + K_u{i})\fmap(:,i))...
          + mu(i)*sigma2^2*K_ul{i}'*(q-p(:,i)) ...
          - mu(i)*sigma2^2*K_ul{i}'*((pinv(M(:,:,i)) + K_u{i})\K_u{i}*(q-p(:,i)) );
      
      % MAP Prediction
      ftst_map(:,i) = (1-sigma2*mu(i))*ftst_map(:,i)+ ...
          mu(i)*sigma2*K_utst{i}'*((pinv(M(:,:,i)) + K_u{i})\fmap(:,i))...
          + mu(i)*sigma2^2*K_utst{i}'*(q-p(:,i)) ...
          - mu(i)*sigma2^2*K_utst{i}'*((pinv(M(:,:,i)) + K_u{i})\K_u{i}*(q-p(:,i)) );
      
      % linear decision
      w_map(:,i) = (1-sigma2*mu(i))*w_map(:,i) + ...
          mu(i)*sigma2*X_U(:,:,i)'*((pinv(M(:,:,i)) + K_u{i})\X_U(:,:,i)*w_map(:,i))...
         + mu(i)*sigma2^2*X_U(:,:,i)'*(q-p(:,i)) ...
          - mu(i)*sigma2^2*X_U(:,:,i)'*((pinv(M(:,:,i)) + K_u{i})\K_u{i}*(q-p(:,i)) );
 ...
 for ii=1:nV
  b(:,ii) = fpred(:,ii).*y_L;
  P(:,:,ii) =  (rho*K_l{ii} - (K_ul{ii}'*((pinv(M(:,:,ii))/sigma2 + K_u{ii})\K_ul{ii}))).*(y_L*y_L');
  [U(:,:,ii), S(:,:,ii)] = eig(P(:,:,ii));

  % call qudratic-solver to solve it
  [alpha, status] = quadr_svm(P(:,:,ii), b(:,ii),param_qu);
%   cvx_begin sdp
%    cvx_precision high
...
for i=1:nV
    ...
    fjointu(:,i) =  fmap(:,i) + sigma2*K_ul{i}*(y_L.*dual_alpha(:,i)) ...
                             - sigma2*K_u{i}*((pinv(M(:,:,i))/sigma2 + K_u{i})\K_ul{i})*(y_L.*dual_alpha(:,i));
                         
    %lableled part
    fjointl(:,i) =  fpred(:,i)+ sigma2*K_l{i}*(y_L.*dual_alpha(:,i)) ...
                             - sigma2*K_ul{i}'*((pinv(M(:,:,i))/sigma2 + K_u{i})\K_ul{i})*(y_L.*dual_alpha(:,i));
    
    %Prediction  
    fjoint_tst(:,i) =  ftst_map(:,i)+ sigma2*K_ltst{i}'*(y_L.*dual_alpha(:,i)) ...
                             - sigma2*K_utst{i}'*((pinv(M(:,:,i))/sigma2 + K_u{i})\K_ul{i})*(y_L.*dual_alpha(:,i));
                         
                         
     wjoint(:,i) = w_map(:,i) + X_L(:,:,i)'*(y_L.*dual_alpha(:,i)) ...
                             - sigma2*X_U(:,:,i)'*((pinv(M(:,:,i))/sigma2 + K_u{i})\K_ul{i})*(y_L.*dual_alpha(:,i));                  
