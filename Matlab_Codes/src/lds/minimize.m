...
% Modified Olivier Chapelle 30/09/2004 [stopping criterion and interface]
...
while i < abs(length)                                      % while not finished
  ...
  if success                                         % if line search succeeded
    ...
    if param.verb > 1
      fprintf(['\t%s %6i;  Value %4.6e\t Directional derivative: %4.6e\t' ...
               'Search direction %4.6e\n'], S, i, f1, d2, max(abs(s)));
    end;  
    if (max(abs(s)) < param.tolX) | (-d2 < param.tolFun)
