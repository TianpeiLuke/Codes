...
if choice ==1 
   src_org = strcat(upper_org2, '/MFCC_win32ms_dim_red');
elseif choice ==2
   src_org = strcat(upper_org2,'/MFCC_win186ms_dim_red'); 
elseif choice ==3
   src_org = strcat(upper_org2, '/MFCC_win32ms_envelop_dim_red');
end
%dest_org = src_org;
...
%% load the dictionary and index system directly 
display('Loading... ') 
if sel==1
...
% record of ground truth (label) and subject index, corrupt index
...
%            err1 = err1 + norm(Y{nC} - D_h_train{chanIndx(nC)}(feaSel,trainSamples)*W(1:nh_train,(nC-1)*nSegs+1 : nC*nSegs ), 'fro'  )^2;   % for training date 09       
       err2 = err2 + norm(test_batch{i} - ...
           train_h_ha{i}(:,partition{2})*W(partition{2}, range ), 'fro' )^2; 
    end
...
ha_acc(nn) = 1- sum(error(nn,NmSubPerDictTest_h+1:nTotalTestSub))/length([NmSubPerDictTest_h+1:nTotalTestSub]);
...
Note = 'Redo Nam algorithm, test on 3 randomly selected dataset with DEC 10 as training set and DEC 09 as testing, data are corrupted by noise ';
...
trial = 12;
save(strcat('decVec_cepsFeas_ALM_train10','_Ndataset_',num2str(nDataset),'_trial_', num2str(trial) ,'.mat'), 'opts', ...
     'h_acc', 'ha_acc', 'overall_acc', 'vDec','error', ...
     'mean_h_acc', 'mean_ha_acc', 'mean_overall_acc',...
      'var_h_acc',  'var_ha_acc',  'var_overall_acc',...
    'Note', 'W_record', 'nDataset', ...
     'test_info', 'Train_metafilelist', 'Test_metafilelist', ...  
     'n_train_h', 'n_test_h',...
     'rec_error', 'label_record','subject_record', 'corrupt_record', ...
     'nSampleTest', 'nSampleTrain', 'nTotalTestSub',...
     'testChan',...
     'NmSubPerDictTrain','NmSubPerDictTest_h', 'NmSubPerDictTest_ha',...
